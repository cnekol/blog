...
- `main` 受规则集保护：只能经 PR 合并，CI「Lint and build」必须通过，禁止强推。
  每项改动从最新的 `origin/main` 开新分支，分支名按内容取（如 `docs/…`、`fix/…`、`chore/…`）。
- 本地需要 Node 24（见 `.node-version`）和 pnpm 12（见 `packageManager`）。云端开发容器默认的 Node 可能更旧，先装好 24 再跑 `pnpm`。
- 开工前先读 `docs/roadmap.md` 的「待办」和「各平台现状」；做完一项就更新它。
- Cloudflare 构建设置与构建日志只能在控制台看，连接器读不到。构建失败时向站长要日志原文，不要凭猜测改配置。
- 用工具创建 PR、评论时，如果工具自动追加了署名或页脚，发出后立即编辑删掉（见下方协作约定）。
